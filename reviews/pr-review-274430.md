...
2. **Take the low-friction Pattern-1 drops in this PR** (banderror's priority, zero coordination needed — both fall back to `@elastic/security-solution`):
   - Common UI components/hooks: `callouts`, `health_truncate_text`, `links_to_docs`, `ml_popover`, `missing_privileges`, `popover_items`, `use_form_with_warnings`.
   - Shared API-integration test helpers: `detections_response/utils`, `.../telemetry`, `.../user_roles`, `sources`, `config/services/detections_response`.

3. **Defer the threat-hunting-impacting drops to a coordinated follow-up.** cypress shared-commons (`support`, `objects`, `screens/common`, `fixtures`, `helpers`) and `server/routes`, `server/utils`, `common/test` would become **threat-hunting-only** post-merge (no base owner fallback). Get threat-hunting's explicit OK first — don't fold these into the rename.

4. **Handle the platform `kbn-*` packages in the regen step (step 1), not by hand.** When updating their `kibana.jsonc`, decide per-package whether to migrate to `engineering` or drop: `kbn-rule-data-utils` can simply drop us (3 co-owners remain); the sole-owned `kbn-openapi-*`, `kbn-zod-helpers` need a real new owner or they orphan.
   - **Exclude `kbn-change-history`** — keep it on `engineering` for now. Kibana core has indicated they'll take ownership once we finish delivering the feature, so it's a planned hand-off later, not a drop candidate here.
