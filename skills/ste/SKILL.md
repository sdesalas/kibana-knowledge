---
name: ste
description: "Rewrites English into Simplified Technical English (ASD-STE100). Use only when the user invokes /ste. Do not apply from ambient context."
...
Use this skill **only** when the user invokes `/ste`. or asks for "STE", "STE100", or "Simplified Technical English". Do not apply it unprompted.

Typical prompted uses:

- Rewrite an agent's output, tool description, error message, or inter-agent instruction so another system cannot misread it.
- Rewrite a prompt or system message to remove ambiguity.
- Produce a **before/after** comparison showing which rule was violated and how the rewrite fixes it. Ask for it — the default output is the rewritten text alone (see Output Format).
