---
name: qaf
description: Create and manage Elastic Cloud Hosted (ECH) deployments with the local QAF CLI. Use when the user invokes /qaf, or asks to create, list, describe, upgrade, register, deregister, or remove ECH / Elastic Cloud deployments, compare main vs a PR, pick a kibana-cloud image, or clean up QAF-registered clusters.
---

# QAF — Elastic Cloud Hosted deployments

Operational recipe for the local [QAF](https://github.com/elastic/qaf) CLI. Install and auth are documented upstream:
...
The user says `/qaf`, or asks to create / manage / compare / tear down ECH deployments via QAF.

## Agent rules

- **Auth is the user's job.** If a command 401s or secrets are missing, tell them the Auth steps and wait. Do not read Vault, copy tokens, write `~/.elastic/cloud.json`, or pass `EC_API_KEY` around.
- **Ask before resolving a Kibana image.** Latest snapshot of `main`? A PR number? Both? Do not guess.
- **Ask before cleanup.** `list` first, then ask which names to drop. Never `remove-all` unless they clearly want every registered cluster destroyed.
- `deregister` ≠ `remove`. `remove` shuts down the ECH cluster (stops billing). `deregister` only forgets it in `~/.qaf/data`.
- A 401 on `list` / `describe` is a stale Cloud API key — not a deleted cluster. Gone is a 410 on `describe`.

## 1. CLI map

`qaf` is a Typer CLI. Nested `--help` is the source of truth if something is not covered here:
...
- Auth: Vault login **and** Elastic Cloud API keys (see below).
- A plan file at `~/.qaf/config/cloud_plans/<plan>.yml`. QAF loads `EC_PLAN=foo` as `foo.yml`.

### Auth — suggest it, do not do it
...
2. Elastic Cloud API keys in `~/.elastic/cloud.json`. Need a key for the environment (`production`, `staging`, or `qa`). Create / copy a production key from [cloud.elastic.co/organization/keys](https://cloud.elastic.co/organization/keys) and put it under `api_key.production`.

If `qaf elastic-cloud deployments list` 401s (`The supplied authentication is invalid`), the key in `cloud.json` is stale. Same fix: suggest the user update it from that page, then retry `list`.
...
Confirm the `serverUrl` in **the user's** plan is still the live monitoring cluster before relying on it. The host can change.
...
Create as many as needed. Each name becomes `elastic.apm.environment` if the plan templates it.
...
| --- | --- |
| `list` / `describe` 401 (`supplied authentication is invalid`) | Stale Cloud API key. Suggest the Auth steps; wait for the user. Not a deleted cluster. |
...
If the user asks to clean up, **list first**, then ask which names to drop.
