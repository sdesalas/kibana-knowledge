#!/usr/bin/env bash
#
# Same as check-tasks.sh but also prints runAt / scheduledAt for the first 20
# security-solution tasks in the task manager index.
#
set -e
...
# 5. Fetch runAt / scheduledAt for the first 20 security-solution tasks.
runtime_json=$(curl -s -u "$AUTH" \
  -H 'content-type: application/json' \
  "$ES_URL/.kibana_task_manager/_search?size=20&filter_path=hits.hits._source.task" \
  -d '{
    "query": { "prefix": { "task.taskType": "alerting:siem." } },
    "_source": ["task.taskType", "task.runAt", "task.scheduledAt", "task.status", "task.enabled"]
  }')

echo "6."

# 6. Print a small report. All counts should be equal.
printf 'rules:           %s\n' "$total"
...
jq -r '
  ...
  (.hits.hits[]?._source.task
    | [.taskType, (.status // "-"), (.enabled|tostring), .runAt, .scheduledAt])
  | @tsv
' <<<"$runtime_json" | column -t -s $'\t'
