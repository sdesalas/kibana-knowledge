...
CANDIDATES: dict[str, str] = {
    ...
    "server/utils":                    "x-pack/solutions/security/plugins/security_solution/server/utils/",
    ...
    "api_integration/detections_response/telemetry": "x-pack/solutions/security/test/security_solution_api_integration/test_suites/detections_response/telemetry/",
    "api_integration/detections_response/user_roles":"x-pack/solutions/security/test/security_solution_api_integration/test_suites/detections_response/user_roles/",
    "api_integration/sources":         "x-pack/solutions/security/test/security_solution_api_integration/test_suites/sources/",
    "api_integration/services/detections_response":  "x-pack/solutions/security/test/security_solution_api_integration/config/services/detections_response/",
    "common/test":                     "x-pack/solutions/security/plugins/security_solution/common/test/",
    # Tier 1b - shared Cypress commons (3-way shared with detection-engine + threat-hunting)
    "cypress/fixtures":                "x-pack/solutions/security/test/security_solution_cypress/cypress/fixtures/",
    "cypress/helpers":                 "x-pack/solutions/security/test/security_solution_cypress/cypress/helpers/",
    "cypress/objects":                 "x-pack/solutions/security/test/security_solution_cypress/cypress/objects/",
    "cypress/screens/common":          "x-pack/solutions/security/test/security_solution_cypress/cypress/screens/common/",
    "cypress/support":                 "x-pack/solutions/security/test/security_solution_cypress/cypress/support/",
...
def _compile_codeowners_pattern(pattern: str) -> re.Pattern[str]:
    ...
    if anchored:
        ...
        prefix = r"(?:\A|/)"
    # CODEOWNERS lets a directory-shaped pattern match everything below it,
    # but a single-* wildcard does NOT descend (it stops at slashes). So a
    # pattern ending in single `*` (not `**`) must match exactly one path
    # segment with no descent.
    ends_double_star = p.endswith("**")
    ends_single_star = p.endswith("*") and not ends_double_star
    if ends_single_star:
        suffix = r"\Z"
    else:
        suffix = r"(?:/.*)?\Z"
...
def main() -> int:
    ...
    pr_list_path = OUT_DIR / "codeowners_pr_savings_links.md"
    with pr_list_path.open("w") as fh:
        ...
        for name in CANDIDATES:
            ...
            fh.write("\n")
    print(f"Wrote per-candidate PR lists -> {pr_list_path}", file=sys.stderr)
    print()
